...
#define CE_WINDOW_ADDRESS_5424 \
	((SOC_WFSS_CE_REG_BASE >> WINDOW_SHIFT) & WINDOW_VALUE_MASK)

#define UMAC_WINDOW_ADDRESS_5424 \
	((UMAC_BASE >> WINDOW_SHIFT) & WINDOW_VALUE_MASK)

#define WINDOW_CONFIGURATION_VALUE_5424 \
	((CE_WINDOW_ADDRESS_5424 << 6) |\
	 (UMAC_WINDOW_ADDRESS_5424 << 12) | \
	 WINDOW_ENABLE_BIT)

/* For Berryllium sw2rxdma ring size increased to 20 bits */
#define HAL_RXDMA_MAX_RING_SIZE_BE 0xFFFFF

#include "hal_5424_rx.h"
#include "hal_5424_tx.h"
#include "hal_be_rx_tlv.h"
#include <hal_be_generic_api.h>

#define LINK_DESC_SIZE (NUM_OF_DWORDS_RX_MSDU_LINK << 2)
#define PMM_REG_BASE_QCA5424 0xB500FC

/**
 * hal_get_link_desc_size_5424(): API to get the link desc size
 *
 * Return: uint32_t
 */
static uint32_t hal_get_link_desc_size_5424(void)
{
	return LINK_DESC_SIZE;
}

/**
 * hal_rx_get_tlv_5424(): API to get the tlv
 *
 * @rx_tlv: TLV data extracted from the rx packet
 * Return: uint8_t
 */
static uint8_t hal_rx_get_tlv_5424(void *rx_tlv)
{
	return HAL_RX_GET(rx_tlv, PHYRX_RSSI_LEGACY, RECEIVE_BANDWIDTH);
}

/**
 * hal_rx_wbm_err_msdu_continuation_get_5424 () - API to check if WBM
 ...
 */
uint8_t hal_rx_wbm_err_msdu_continuation_get_5424(void *wbm_desc)
{
 ...
 */
static inline
void hal_rx_proc_phyrx_other_receive_info_tlv_5424(void *rx_tlv_hdr,
				     void *ppdu_info_hdl)
{
...
static inline
void hal_rx_get_bb_info_5424(void *rx_tlv, void *ppdu_info_hdl)
{
...
static inline
void hal_rx_get_rtt_info_5424(void *rx_tlv, void *ppdu_info_hdl)
{
	struct hal_rx_ppdu_info *ppdu_info	= ppdu_info_hdl;
...
/**
 * hal_rx_dump_mpdu_start_tlv_5424() - dump RX mpdu_start TLV in structured
 ...
 * Return: void
 */
static inline void hal_rx_dump_mpdu_start_tlv_5424(void *mpdustart,
		uint8_t dbg_level)
{
...
/**
 * hal_rx_dump_msdu_end_tlv_5424() - dump RX msdu_end TLV in structured
 *				 human readable format.
 ...
 * Return: void
 */
static void hal_rx_dump_msdu_end_tlv_5424(void *msduend,
		uint8_t dbg_level)
{
...
/**
 * hal_reo_status_get_header_5424() - Process reo desc info
 *
 ...
 *
 */
static void hal_reo_status_get_header_5424(hal_ring_desc_t ring_desc,
		int b, void *h1)
{
...
static
void *hal_rx_msdu0_buffer_addr_lsb_5424(void *link_desc_va)
{
	return (void *)HAL_RX_MSDU0_BUFFER_ADDR_LSB(link_desc_va);
}

static
void *hal_rx_msdu_desc_info_ptr_get_5424(void *msdu0)
{
	return (void *)HAL_RX_MSDU_DESC_INFO_PTR_GET(msdu0);
}

static
void *hal_ent_mpdu_desc_info_5424(void *ent_ring_desc)
{
	return (void *)HAL_ENT_MPDU_DESC_INFO(ent_ring_desc);
}

static
void *hal_dst_mpdu_desc_info_5424(void *dst_ring_desc)
{
	return (void *)HAL_DST_MPDU_DESC_INFO(dst_ring_desc);
}

/**
 * hal_reo_config_5424(): Set reo config parameters
 * @soc: hal soc handle
 ...
 */
static void
hal_reo_config_5424(struct hal_soc *soc,
			   uint32_t reg_val,
...
/**
 * hal_rx_msdu_desc_info_get_ptr_5424() - Get msdu desc info ptr
 * @msdu_details_ptr: Pointer to msdu_details_ptr
 *
 * Return: Pointer to rx_msdu_desc_info structure.
 *
 */
static void *hal_rx_msdu_desc_info_get_ptr_5424(void *msdu_details_ptr)
{
	return HAL_RX_MSDU_DESC_INFO_GET(msdu_details_ptr);
}

/**
 * hal_rx_link_desc_msdu0_ptr_5424 - Get pointer to rx_msdu details
 * @link_desc: Pointer to link desc
 *
 * Return: Pointer to rx_msdu_details structure
 *
 */
static void *hal_rx_link_desc_msdu0_ptr_5424(void *link_desc)
{
	return HAL_RX_LINK_DESC_MSDU0_PTR(link_desc);
}

/**
 * hal_get_window_address_5424(): Function to get hp/tp address
...
static inline qdf_iomem_t hal_get_window_address_5424(struct hal_soc *hal_soc,
		qdf_iomem_t addr)
{
...
static inline void hal_write_window_register(struct hal_soc *hal_soc)
{
	...
	qdf_iowrite32(hal_soc->dev_base_addr + WINDOW_REG_ADDRESS,
			WINDOW_CONFIGURATION_VALUE_5424);
}

static
void hal_compute_reo_remap_ix2_ix3_5424(uint32_t *ring, uint32_t num_rings,
...
/**
 * hal_rx_flow_write_fse_metadata_5424() - Update fse metadata in HW FST
 ...
 */
static void *
hal_rx_flow_write_fse_metadata_5424(uint8_t *rx_fst, uint32_t table_offset,
				    uint8_t *rx_flow)
{
...
/**
 * hal_rx_flow_setup_fse_5424() - Setup a flow search entry in HW FST
 * @rx_fst: Pointer to the Rx Flow Search Table
 * @table_offset: offset into the table where the flow is to be setup
 * @rx_flow: Flow Parameters
 *
 ...
 */
static void *
hal_rx_flow_setup_fse_5424(uint8_t *rx_fst, uint32_t table_offset,
		uint8_t *rx_flow)
{
	struct hal_rx_fst *fst = (struct hal_rx_fst *)rx_fst;
	struct hal_rx_flow *flow = (struct hal_rx_flow *)rx_flow;
...
static inline uint8_t hal_rx_msdu_get_keyid_5424(uint8_t *buf)
{
...
/**
 * hal_rx_tlv_get_freq_5424() - API to get the frequency of operating
...
static inline uint32_t hal_rx_tlv_get_freq_5424(uint8_t *buf)
{
	struct rx_pkt_tlvs *rx_pkt_tlvs = (struct rx_pkt_tlvs *)buf;
	uint32_t freq;
...
/**
 * hal_rx_mpdu_start_sw_peer_id_get_5424() - Retrieve sw peer_id
 * @buf: network buffer
 *
 * Return: sw peer_id
 */
static inline uint32_t hal_rx_mpdu_start_sw_peer_id_get_5424(uint8_t *buf)
{
	struct rx_pkt_tlvs *rx_pkt_tlvs = (struct rx_pkt_tlvs *)buf;
...
#ifdef CONFIG_WORD_BASED_TLV
/**
 * hal_rx_priv_info_set_in_tlv_5424() - Save the private info to
 ...
 * Return: void
 */
static inline void hal_rx_priv_info_set_in_tlv_5424(uint8_t *buf,
						    uint8_t *priv_data,
						    uint32_t len)
{
...
/**
 * hal_rx_priv_info_get_from_tlv_5424() - retrieve the private data from
 *                             the reserved bytes of rx_tlv_hdr.
 * @buf: start of rx_tlv_hdr
 ...
 * Return: void
 */
static inline void hal_rx_priv_info_get_from_tlv_5424(uint8_t *buf,
						      uint8_t *priv_data,
						      uint32_t len)
{
	struct rx_pkt_tlvs *pkt_tlvs = (struct rx_pkt_tlvs *)buf;
...
/**
 * hal_rx_dump_pkt_hdr_tlv_5424(): dump RX pkt header TLV in hex format
 * @pkt_tlvs: pointer the pkt_hdr_tlv in pkt.
 * @dbg_level: log level.
 *
 * Return: void
 */
static inline void hal_rx_dump_pkt_hdr_tlv_5424(struct rx_pkt_tlvs *pkt_tlvs,
		uint8_t dbg_level)
{
...
/**
 * hal_rx_dump_pkt_tlvs_5424(): API to print RX Pkt TLVS qca5424
 ...
 */
#ifdef CONFIG_WORD_BASED_TLV
static void hal_rx_dump_pkt_tlvs_5424(hal_soc_handle_t hal_soc_hdl,
		uint8_t *buf, uint8_t dbg_level)
{
	...
	hal_rx_dump_msdu_end_tlv_5424(msdu_end, dbg_level);
	hal_rx_dump_mpdu_start_tlv_5424(mpdu_start, dbg_level);
	hal_rx_dump_pkt_hdr_tlv_5424(pkt_tlvs, dbg_level);
}
#else
static void hal_rx_dump_pkt_tlvs_5424(hal_soc_handle_t hal_soc_hdl,
		uint8_t *buf, uint8_t dbg_level)
...
#define HAL_NUM_TCL_BANKS_5424 24

/**
 * hal_cmem_write_5424() - function for CMEM buffer writing
 ...
 * Return: None.
 */
static void hal_cmem_write_5424(hal_soc_handle_t hal_soc_hdl,
		uint32_t offset,
		uint32_t value)
{
...
/**
 * hal_tx_get_num_tcl_banks_5424() - Get number of banks in target
 *
 * Returns: number of bank
 */
static uint8_t hal_tx_get_num_tcl_banks_5424(void)
{
	return HAL_NUM_TCL_BANKS_5424;
}

	static
void hal_compute_reo_remap_ix0_5424(struct hal_soc *soc)
{
	uint32_t remap0;
...
static void hal_reo_setup_5424(struct hal_soc *soc, void *reoparams,
		int qref_reset)
{
	uint32_t reg_val;
	...
	hal_reo_config_5424(soc, reg_val, reo_params);
	...
	if (reo_params->rx_hash_enabled) {
		hal_compute_reo_remap_ix0_5424(soc);
		HAL_REG_WRITE(soc,
...
static uint16_t hal_get_rx_max_ba_window_qca5424(int tid)
{
	return HAL_RX_BA_WINDOW_1024;
}

/**
 * hal_qca5424_get_reo_qdesc_size()- Get the reo queue descriptor size
 ...
 */
static uint32_t hal_qca5424_get_reo_qdesc_size(uint32_t ba_window_size, int tid)
{
	/* Hardcode the ba_window_size to HAL_RX_MAX_BA_WINDOW for
	 * NON_QOS_TID until HW issues are resolved.
	 */
	if (tid != HAL_NON_QOS_TID)
		ba_window_size = hal_get_rx_max_ba_window_qca5424(tid);
...
/**
 * hal_rx_tlv_msdu_done_copy_get_5424() - Get msdu done copy bit from rx_tlv
 *
 * @buf: pointer the tx_tlv
 *
 * Returns: msdu done copy bit
 */
static inline uint32_t hal_rx_tlv_msdu_done_copy_get_5424(uint8_t *buf)
{
 ...
 */
static inline
uint32_t hal_read_pmm_scratch_reg_5424(struct hal_soc *soc,
				       enum hal_scratch_reg_enum reg_enum)
{
	uint32_t val = 0;
...
/**
 * hal_get_tsf2_scratch_reg_qca5424(): API to read tsf2 scratch register
 *
 * @hal_soc_hdl: HAL soc context
 * @mac_id: mac id
 * @value: Pointer to update tsf2 value
 ...
 * Return: void
 */
static void hal_get_tsf2_scratch_reg_qca5424(hal_soc_handle_t hal_soc_hdl,
		uint8_t mac_id, uint64_t *value)
{
	struct hal_soc *soc = (struct hal_soc *)hal_soc_hdl;
	uint32_t offset_lo, offset_hi;
	...
	offset_lo = hal_read_pmm_scratch_reg_5424(soc, enum_lo);

	offset_hi = hal_read_pmm_scratch_reg_5424(soc, enum_hi);

	*value = ((uint64_t)(offset_hi) << 32 | offset_lo);
}

/**
 * hal_get_tqm_scratch_reg_qca5424(): API to read tqm scratch register
 *
 * @hal_soc_hdl: HAL soc context
 * @value: Pointer to update tqm value
 *
...
static void hal_get_tqm_scratch_reg_qca5424(hal_soc_handle_t hal_soc_hdl,
		uint64_t *value)
{
	struct hal_soc *soc = (struct hal_soc *)hal_soc_hdl;
	uint32_t offset_lo, offset_hi;

	offset_lo = hal_read_pmm_scratch_reg_5424(soc,
						PMM_TQM_CLOCK_OFFSET_LO_US);

	offset_hi = hal_read_pmm_scratch_reg_5424(soc,
						PMM_TQM_CLOCK_OFFSET_HI_US);

	*value = ((uint64_t)(offset_hi) << 32 | offset_lo);
}

static void hal_hw_txrx_ops_attach_qca5424(struct hal_soc *hal_soc)
{
	/* init and setup */
	...
	hal_soc->ops->hal_get_window_address = hal_get_window_address_5424;
	hal_soc->ops->hal_cmem_write = hal_cmem_write_5424;

	/* tx */
	hal_soc->ops->hal_tx_set_dscp_tid_map = hal_tx_set_dscp_tid_map_5424;
	hal_soc->ops->hal_tx_update_dscp_tid = hal_tx_update_dscp_tid_5424;
	hal_soc->ops->hal_tx_comp_get_status =
		hal_tx_comp_get_status_generic_be;
	hal_soc->ops->hal_tx_init_cmd_credit_ring =
		hal_tx_init_cmd_credit_ring_5424;
	hal_soc->ops->hal_tx_set_ppe_cmn_cfg = hal_tx_set_ppe_cmn_config_5424;
	hal_soc->ops->hal_tx_set_ppe_vp_entry = hal_tx_set_ppe_vp_entry_5424;
	hal_soc->ops->hal_ppeds_cfg_ast_override_map_reg =
		hal_ppeds_cfg_ast_override_map_reg_5424;
	hal_soc->ops->hal_tx_set_ppe_pri2tid = hal_tx_set_ppe_pri2tid_map_5424;
	hal_soc->ops->hal_tx_update_ppe_pri2tid =
		hal_tx_update_ppe_pri2tid_5424;
	hal_soc->ops->hal_tx_dump_ppe_vp_entry = hal_tx_dump_ppe_vp_entry_5424;
	hal_soc->ops->hal_tx_get_num_ppe_vp_tbl_entries =
		hal_tx_get_num_ppe_vp_tbl_entries_5424;
	hal_soc->ops->hal_tx_enable_pri2tid_map =
		hal_tx_enable_pri2tid_map_5424;
	hal_soc->ops->hal_tx_config_rbm_mapping_be =
		hal_tx_config_rbm_mapping_be_5424;
	...
	hal_soc->ops->hal_rx_get_tlv = hal_rx_get_tlv_5424;
	hal_soc->ops->hal_rx_proc_phyrx_other_receive_info_tlv =
		hal_rx_proc_phyrx_other_receive_info_tlv_5424;

	hal_soc->ops->hal_rx_dump_msdu_end_tlv = hal_rx_dump_msdu_end_tlv_5424;
	hal_soc->ops->hal_rx_dump_mpdu_start_tlv =
		hal_rx_dump_mpdu_start_tlv_5424;
	hal_soc->ops->hal_rx_dump_pkt_tlvs = hal_rx_dump_pkt_tlvs_5424;

	hal_soc->ops->hal_get_link_desc_size = hal_get_link_desc_size_5424;
	...
	hal_soc->ops->hal_rx_msdu_desc_info_get_ptr =
		hal_rx_msdu_desc_info_get_ptr_5424;
	hal_soc->ops->hal_rx_link_desc_msdu0_ptr =
		hal_rx_link_desc_msdu0_ptr_5424;
	hal_soc->ops->hal_reo_status_get_header =
		hal_reo_status_get_header_5424;
	...
	hal_soc->ops->hal_rx_msdu0_buffer_addr_lsb =
		hal_rx_msdu0_buffer_addr_lsb_5424;
	hal_soc->ops->hal_rx_msdu_desc_info_ptr_get =
		hal_rx_msdu_desc_info_ptr_get_5424;
	hal_soc->ops->hal_ent_mpdu_desc_info = hal_ent_mpdu_desc_info_5424;
	hal_soc->ops->hal_dst_mpdu_desc_info = hal_dst_mpdu_desc_info_5424;
	...
	hal_soc->ops->hal_rx_flow_write_fse_metadata =
		hal_rx_flow_write_fse_metadata_5424;
	hal_soc->ops->hal_reo_config = hal_reo_config_5424;
...
#if defined(WLAN_CFR_ENABLE) && defined(WLAN_ENH_CFR_ENABLE)
	hal_soc->ops->hal_rx_get_bb_info = hal_rx_get_bb_info_5424;
	hal_soc->ops->hal_rx_get_rtt_info = hal_rx_get_rtt_info_5424;
#else
	...
	hal_soc->ops->hal_rx_wbm_err_msdu_continuation_get =
		hal_rx_wbm_err_msdu_continuation_get_5424;
...
#endif
	hal_soc->ops->hal_rx_flow_setup_fse = hal_rx_flow_setup_fse_5424;
	...
	hal_soc->ops->hal_compute_reo_remap_ix2_ix3 =
		hal_compute_reo_remap_ix2_ix3_5424;
	...
	hal_soc->ops->hal_tx_get_num_tcl_banks = hal_tx_get_num_tcl_banks_5424;
	...
	hal_soc->ops->hal_rx_tlv_msdu_done_get =
		hal_rx_tlv_msdu_done_copy_get_5424;
	...
	hal_soc->ops->hal_rx_msdu_get_keyid = hal_rx_msdu_get_keyid_5424;
	hal_soc->ops->hal_rx_tlv_get_freq = hal_rx_tlv_get_freq_5424;
#ifdef CONFIG_WORD_BASED_TLV
	hal_soc->ops->hal_rx_priv_info_set_in_tlv =
		hal_rx_priv_info_set_in_tlv_5424;
	hal_soc->ops->hal_rx_priv_info_get_from_tlv =
		hal_rx_priv_info_get_from_tlv_5424;
#else
	hal_soc->ops->hal_rx_priv_info_set_in_tlv =
	...
	hal_soc->ops->hal_reo_setup = hal_reo_setup_5424;
	...
	hal_soc->ops->hal_get_rx_max_ba_window =
		hal_get_rx_max_ba_window_qca5424;
	hal_soc->ops->hal_get_reo_qdesc_size = hal_qca5424_get_reo_qdesc_size;
	/* TX MONITOR */
	...
	hal_soc->ops->hal_get_tsf2_scratch_reg =
		hal_get_tsf2_scratch_reg_qca5424;
	hal_soc->ops->hal_get_tqm_scratch_reg =
		hal_get_tqm_scratch_reg_qca5424;
	hal_soc->ops->hal_tx_ring_halt_set = hal_tx_ppe2tcl_ring_halt_set_5424;
	hal_soc->ops->hal_tx_ring_halt_reset =
		hal_tx_ppe2tcl_ring_halt_reset_5424;
...
struct hal_hw_srng_config hw_srng_table_5424[] = {
	...
	{ /* TCL_CMD/CREDIT */
		/* qca8074v2 and qca5424 uses this ring for data commands */
...
/**
 * hal_srng_hw_reg_offset_init_qca5424() - Initialize the HW srng reg offset
 *				applicable only for qca5424
 * @hal_soc: HAL Soc handle
 *
 * Return: None
 */
static inline void hal_srng_hw_reg_offset_init_qca5424(struct hal_soc *hal_soc)
{
...
/*
 * hal_reo_config_reo2ppe_dest_info_5424() - Configure reo2ppe dest info
 * @hal_soc_hdl: HAL SoC Context
 *
 * Return: None.
 */
	static inline
void hal_reo_config_reo2ppe_dest_info_5424(hal_soc_handle_t hal_soc_hdl)
{
...
static void hal_hw_txrx_ops_override_qca5424(struct hal_soc *hal_soc)
{
	hal_soc->ops->hal_reo_config_reo2ppe_dest_info =
		hal_reo_config_reo2ppe_dest_info_5424;
...
/**
 * hal_qca5424_attach()- Attach 5424 target specific hal_soc ops,
 *			  offset and srng table
 * @hal_soc: hal_soc handle
 *
 * Return: void
 */
void hal_qca5424_attach(struct hal_soc *hal_soc)
{
	hal_soc->hw_srng_table = hw_srng_table_5424;

	hal_srng_hw_reg_offset_init_generic(hal_soc);
	hal_srng_hw_reg_offset_init_qca5424(hal_soc);

	hal_hw_txrx_default_ops_attach_be(hal_soc);
	hal_hw_txrx_ops_attach_qca5424(hal_soc);
