			  ...
			  uint32_t num_req_buffers)
{
	 ...
	 */
	total_pages = (nr_descs * sizeof(*nf_info)) / qdf_page_size;

	/*
	 * Add an extra page to store the remainder if any
	 */
	if ((nr_descs * sizeof(*nf_info)) % qdf_page_size)
		total_pages++;
	nf_info = qdf_mem_malloc(qdf_page_size);
	if (!nf_info) {
	...
	nbuf_ptrs_per_page = qdf_page_size / sizeof(*nf_info);

	for (page_idx = 0; page_idx < total_pages; page_idx++) {
		qdf_mem_zero(nf_info, qdf_page_size);
