...
static void dp_ipa_set_alt_tx_db_paddr(struct dp_soc *soc,
				       struct dp_ipa_resources *ipa_res)
{
	struct hal_srng *wbm_srng = (struct hal_srng *)
			soc->tx_comp_ring[IPA_TX_ALT_COMP_RING_IDX].hal_srng;
...
static QDF_STATUS
dp_ipa_reset_alt_tx_doorbell_pa(struct dp_soc *soc,
				struct dp_ipa_resources *ipa_res)
{
...
#define DP_IPA_SET_ALT_TX_DB_PADDR(soc, ipa_res)

#define DP_IPA_EP_SET_ALT_TX_DB_PA(soc, ipa_res) \
	dp_ipa_set_alt_tx_db_paddr((soc), (ipa_res))

#define DP_IPA_RESET_ALT_TX_DB_PA(soc, ipa_res) \
	dp_ipa_reset_alt_tx_doorbell_pa((soc), (ipa_res))

#else /* !IPA_SET_RESET_TX_DB_PA */
#define DP_IPA_SET_ALT_TX_DB_PADDR(soc, ipa_res) \
	dp_ipa_set_alt_tx_db_paddr(soc, ipa_res)
...
static inline void dp_ipa_unmap_tx_alt_ring_doorbell_paddr(struct dp_pdev *pdev)
{
	return 0;
}
...
QDF_STATUS dp_ipa_enable_pipes(struct cdp_soc_t *soc_hdl, uint8_t pdev_id)
{
	...
	DP_IPA_EP_SET_ALT_TX_DB_PA(soc, ipa_res);
	...
	if (result) {
		...
		DP_IPA_RESET_ALT_TX_DB_PA(soc, ipa_res);
